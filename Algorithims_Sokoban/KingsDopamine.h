...
public:
	...
	struct KingsPair {
		...
		bool operator<(const KingsPair& other) const {
			return aStarPair.first > other.aStarPair.first;
		}
	};

	std::priority_queue<KingsPair> priorityQueue;
	...
	int KingsValue(SeanNode* inputNode, SeanNode FinalNode, int distanceFromRoot) {
		int mapValue = 0;
		...
		for (auto it = inputNode->specialBoxes.begin(); it != inputNode->specialBoxes.end(); ++it) {
			...
			if (*it == *it2) {
				//the king decrees if we get a point its worth traveling down this path the higher the distance the more likely this is the way
				if (capturedPoints == 0) {
					capturedPoints += distanceFromRoot / 2;
				}
				else {//the king decrees as we capture more points to add some extra points but not too many 
					capturedPoints += capturedPoints / 2;
				}
			}
	...
	void KingsDopamineOverhaul(SeanNode* inputNode, int width, int height, std::vector<bool> sokoMind, SeanNode FinalNode, SeanNode CompleteMapNode, std::chrono::time_point<std::chrono::high_resolution_clock> startTime, std::string mapName) {
		//debug
		//int mapAStar = 0;
		...
		while (!inputNode->win) {
			...
			for (int i = 0; i < tempReturn.nodeVector.size(); i++) {
				...
				if (testSet->find(tempHash) != testSet->end()) {
					...
					if (tempReturn.nodeVector[i].first->win) {
						std::string searchAlgorithm = "KINGS_DOPAMINE";
			...
			for (auto it = tempReturn.nodeVector.begin(); it != tempReturn.nodeVector.end(); ++it) {
				tempHash = KingsValue(it->first, FinalNode, distanceFromRoot);
